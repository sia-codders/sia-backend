...
class Helpers {
    ...
    //Detalle: Tranforma en json la respuesta echa por la base de datos
    ...
    private function validar_respuestas_bdd($nombre_controlador, $tokenEnviado, $json) {
        ...
        if ($this->validar_token($tokenEnviado)) {
            ...
            $obj_controlador = new $nombre_controlador($json);            
            //$data_respuesta_base alamacena un JSON
            $data_respuesta_base = $obj_controlador->generar_respuesta();
            ...
            if ($estado_respuesta == $obj_mensaje->mensajeOK) {
                if ($filas_respuesta == $obj_mensaje->numero_cero) {
                    $this->retornar_respuesta($estado_respuesta, $obj_mensaje->NO_existe_coincidencias, $filas_respuesta, $array_respuesta);
                } else {
                    $this->retornar_respuesta($estado_respuesta, $obj_mensaje->SI_existe_coincidencias, $filas_respuesta, $array_respuesta);
                }
            } else {
                $this->retornar_respuesta($estado_respuesta, $obj_mensaje->no_realizacion_busqueda, $filas_respuesta, $array_respuesta);
            }
        } else {
            $this->retornar_respuesta($estado_respuesta, $obj_mensaje->no_tiene_permiso, $filas_respuesta, $array_respuesta);
        }
    }
    ...
    private function validar_dato_numero($posible_numero) {
        return is_numeric($posible_numero);
    }

    //Creado por: Santiago          Fecha: 2021-09-29
    //Detalle: Realiza las validacion de los parametros enviados por el metodo POST, 
    //que exista concordancia entre el arreglo y el nombre de JSON, igual forma para las cabeceras
    private function validar_parametros_envioHTTP_POST($arreglo_post, $nombre_json, $arreglo_cabecera, $nombre_token) {
        //Creado Por: Santiago  /  Fecha: 2021-09-26 17:43
        $respuesta = false;
        if (isset($arreglo_post[$nombre_json]) && isset($arreglo_cabecera[$nombre_token])) {
            $respuesta = true;
        }
        return $respuesta;
    }

    private function validar_parametros_envioHTTP_GET($arreglo_get, $nombre_json, $arreglo_cabecera, $nombre_token) {
        //Creado por: Santiago          Fecha: 2021-09-29
        //Detalle: Realiza las validacion de los parametros enviados por el metodo GET
        //Creado Por: Santiago  /  Fecha: 2021-09-26 17:43
        $respuesta = false;
        if (isset($arreglo_get[$nombre_json]) && isset($arreglo_cabecera[$nombre_token])) {
            $respuesta = true;
        }
        return $respuesta;
    }
    ...
    //Detalle: Realiza las validaciones del metodo HTTP ENVIADO, si es o no valido para trabajar acorde el controlador 
    public function validar_metodoHTTP_POST($nombre_controlador, $metodo, $arreglo_post, $arreglo_cabecera, $nombre_token) {        
        ...
        if ($metodo == 'POST') {
            ...
            if ($this->validar_parametros_envioHTTP_POST($arreglo_post, $nombre_json, $arreglo_cabecera, $nombre_token)) {
                ...
                $this->validar_respuestas_bdd($nombre_controlador, $tokenEnviado, $json);
            } else {
